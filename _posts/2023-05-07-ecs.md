...
- <a href="#implementation" class="content">Part 2. My implementation</a>
   ...
   - <a href="#implementation_scene" class="content">Second iteration</a>
      ...
      - <a href="#improvements" class="content">Potential improvements: SceneView, Archetypes</a>
...
Secondly, we need to hold all active entities of the scene. vector container is appropriate for this purpose (m_entitiesSignatures).
...
But we would need to iterate unordered_map (m_entities) in the ComponentPool, which is not a good idea because we can have hundreds of thousands of entities. So, I've decided to add an unordered_map container (m_entitiesSignatures) in the Scene class for this purpose. A <b>key</b> will be an <b>Entity id</b> and a <b>value</b> will be a <a href="https://en.wikipedia.org/wiki/Mask_(computing)" target="_blank" class="content"><b>bitmask</b></a>. Every bit in bitmask corresponds to a particular ComponentType.
...
ComponentType& AddComponent(ecs::Entity entity, Args&&... args)
{
   ...
   EL_CORE_ASSERT(entityIt != m_entitiesSignatures.end(), "The entity is not found.");
...
As you may notice, I don't create a Behavior object it the moment of component construction.
